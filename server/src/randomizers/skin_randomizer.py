        ...
        randomizer_pool = {
            weapon: {
                skin: {
                    ...
                    "levels": {
                        level: level_data
                        for level, level_data in skin_data["levels"].items()
                        if level_data["favorite"]
                    },
                    "chromas": {
                        chroma: chroma_data
                        for chroma, chroma_data in skin_data["chromas"].items()
                        if chroma_data["favorite"]
                    },
                }
                for skin, skin_data in weapon_data["skins"].items()
                if skin_data["favorite"]
            }
            for weapon, weapon_data in inventory.items()
            if not weapon_data["locked"]
        }
            ...
            for skin_uuid, skin in weapon.items():
                if len(weapon.values()) > 2:
                    ...
                    for chroma_uuid, chroma in skin["chromas"].items():
                        if (
                            not chroma_uuid in equipped_chroma_ids
                            and len(skin["chromas"]) > 1
                        ):
        ...
        for weapon in loadout["Guns"]:
            if not inventory[weapon["ID"]]["locked"]:
                ...
                if (
                    ...
                    == True
                    and len(randomizer_pool_no_repeats[weapon["ID"]].values()) >= 1
                ):
                    # print(weapon["ID"])
                ...
                if weapon_data != {}:
                    ...
                    skin_uuid = random.choices(
                        list(weapon_data.keys()), weights=weights
                    )[0]
                    ...
                    level_index = (
                        max_level - 1
                    )  # use max level by default unless min chroma level is selected

                    chroma_index = (
                        random.randrange(0, len(skin["chromas"]))
