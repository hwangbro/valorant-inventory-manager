...
class File_Manager:
    ...
    def fetch_inventory():
        ...
        try:
            with open(
                Filepath.get_path(
                    os.path.join(Filepath.get_appdata_folder(), "inventory.json")
                )
            ) as f:
                data = json.load(f)
                try:
    ...
    def fetch_profiles():
        ...
        try:
            with open(
                Filepath.get_path(
                    os.path.join(Filepath.get_appdata_folder(), "profiles.json")
                )
            ) as f:
                data = json.load(f)
                try:
    ...
    def update_file(new_data, destination):
        with open(
            Filepath.get_path(os.path.join(Filepath.get_appdata_folder(), destination)),
            "w",
        ) as f:
