        ...
        for skin_uuid, skin_data in weapon_data["skins"].items():
            ...
            def find_top_unlocked(key):
                for index in range(len(inventory_skin_data[key]) - 1, -1, -1):
                    data = inventory_skin_data[key][
                        list(inventory_skin_data[key].keys())[index]
                    ]
                    if data["unlocked"]:
            ...
            for level_uuid, level_data in skin_data["levels"].items():
                level_data["favorite"] = profile_data[skin_uuid]["levels"][level_uuid][
                    "favorite"
                ]
            for chroma_uuid, chroma_data in skin_data["chromas"].items():
                chroma_data["favorite"] = profile_data[skin_uuid]["chromas"][
                    chroma_uuid
                ]["favorite"]
                    ...
                    "skins": {
                        skin_uuid: {
                            ...
                            "levels": {
                                ...
                                for level_uuid, level_data in skin_data[
                                    "levels"
                                ].items()
                            },
                            "chromas": {
                                ...
                                for chroma_uuid, chroma_data in skin_data[
                                    "chromas"
                                ].items()
                            },
                        }
    ...
    def refresh_profiles():
        ...
        for index, profile in enumerate(Profile_Manager.fetch_profiles()):
            ...
            for weapon_uuid, weapon in inventory["skins"].items():
                ...
                weapon_payload["locked"] = (
                    old_weapon_data["locked"] if old_weapon_data is not None else False
                )
                ...
                for skin_uuid, skin in weapon["skins"].items():
                    if skin["unlocked"]:
                        ...
                        if old_weapon_data is not None:
                            try:
                                existing_skin_data = old_weapon_data.get("skins").get(
                                    skin["uuid"]
                                )
                            except:
                                pass
                        ...
                        skin_payload["favorite"] = (
                            existing_skin_data["favorite"]
                            if existing_skin_data is not None
                            else False
                        )
                        skin_payload["weight"] = (
                            existing_skin_data["weight"]
                            if existing_skin_data is not None
                            else 1
                        )
                        weapon_payload["total_weights"] += (
                            skin_payload["weight"] if skin_payload["favorite"] else 0
                        )
                        ...
                        for level_uuid, level in skin["levels"].items():
                            ...
                            level_payload["favorite"] = (
                                existing_skin_data["levels"][level["uuid"]]["favorite"]
                                if existing_skin_data is not None
                                else False
                            )
                        ...
                        for chroma_uuid, chroma in skin["chromas"].items():
                            ...
                            chroma_payload["favorite"] = (
                                existing_skin_data["chromas"][chroma["uuid"]][
                                    "favorite"
                                ]
                                if existing_skin_data is not None
                                else False
