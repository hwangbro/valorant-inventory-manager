...
from .valclient import ValClient
...
class Client:
    ...
    def __init__(self):
        ...
        self.all_weapon_data = requests.get(
            "https://valorant-api.com/v1/weapons"
        ).json()["data"]
        self.all_buddy_data = requests.get(
            "https://valorant-api.com/v1/buddies"
        ).json()["data"]
        self.content_tiers = requests.get(
    ...
    async def check_connection(self):
        if not system.are_processes_running():
            self.ready = False
            self.client = None
            payload = {"event": "game_not_running", "data": {}}
            ...
            if not self.ready:
                self.connect()
                payload = {"event": "client_connected", "data": True}
                await broadcast(payload)
                print("game running")
                return True
    ...
    def put_weapon(self, **kwargs):
        ...
        Profile_Manager.update_profile_loadout(
            Profile_Manager.SELECTED_PROFILE, self.client.fetch_player_loadout()
        )
        ...
        for weapon in loadout["Guns"]:
            ...
            if weapon_uuid != "2f59173c-4bed-b6c3-2191-dea9b58be9c7":
                if new_loadout[weapon_uuid]["buddy_uuid"] != "":
                    ...
                    weapon["CharmInstanceID"] = new_loadout[weapon_uuid][
                        "buddy_instance_uuid"
                    ]
                    weapon["CharmLevelID"] = new_loadout[weapon_uuid][
                        "buddy_level_uuid"
                    ]
                else:
                    weapon["CharmID"] = None
    ...
    def fetch_loadout(self):
        ...
        for weapon in loadout["Guns"]:
            ...
            weapon_uuid = weapon["ID"]
            weapon_data = next(
                item for item in self.all_weapon_data if item["uuid"] == weapon_uuid
            )
            skin_data = next(
                item
                for item in weapon_data["skins"]
                if item["uuid"] == weapon["SkinID"]
            )
            level_data = next(
                item
                for item in skin_data["levels"]
                if item["uuid"] == weapon["SkinLevelID"]
            )
            chroma_data = next(
                item
                for item in skin_data["chromas"]
                if item["uuid"] == weapon["ChromaID"]
            )
            ...
            try:
                tier_data = next(
                    ...
                    if tier["uuid"] == skin_data["contentTierUuid"]
                )
            except:
                tier_data = {
                    "devName": "Battlepass",
            ...
            if chroma_index == 0 and level_data["displayIcon"] != None:
                if (
                    skin_data["themeUuid"] in COLLECTIONS_WITH_BAD_LEVEL_IMAGES
                    or "Standard" in skin_data["displayName"]
                ):
            ...
            if weapon.get("CharmID"):
                ...
                buddy_data = next(
                    item for item in self.all_buddy_data if item["uuid"] == buddy_uuid
                )
    ...
    async def broadcast_loadout(self):
        ...
        payload = {"event": "loadout_updated", "data": loadout}
        await broadcast(payload)
